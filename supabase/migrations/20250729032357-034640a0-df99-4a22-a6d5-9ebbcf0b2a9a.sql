...
-- Then, assign reactor_ids to any records that don't have one
WITH missing_reactor_ids AS (
  SELECT 
    id,
    row_number() OVER (ORDER BY created_at) + 
    COALESCE((SELECT MAX(CAST(SUBSTRING(reactor_id FROM 2) AS INTEGER)) FROM user_profiles WHERE reactor_id IS NOT NULL), 0) as new_reactor_num
  FROM user_profiles 
