...
random_demographics AS (
  SELECT 
    ...
    (SELECT json_array_elements_text(demo_options) 
     FROM demo_data WHERE demo = 'gender' ORDER BY random() LIMIT 1) as gender,
    -- Random race from demo_prelim
    (SELECT json_array_elements_text(demo_options) 
     FROM demo_data WHERE demo = 'race' ORDER BY random() LIMIT 1) as race,
    -- Random education from demo_prelim
    (SELECT json_array_elements_text(demo_options) 
     FROM demo_data WHERE demo = 'education' ORDER BY random() LIMIT 1) as education,
    -- Random income from demo_prelim
    (SELECT json_array_elements_text(demo_options) 
     FROM demo_data WHERE demo = 'income' ORDER BY random() LIMIT 1) as income,
    -- Random marital status from demo_prelim
    (SELECT json_array_elements_text(demo_options) 
     FROM demo_data WHERE demo = 'marital_status' ORDER BY random() LIMIT 1) as marital_status,
    -- Random registered voter
    (ARRAY['Yes', 'No'])[floor(random() * 2 + 1)] as registered_voter,
    -- Random vote 2024 from demo_prelim
    (SELECT json_array_elements_text(demo_options) 
     FROM demo_data WHERE demo = 'vote_2024' ORDER BY random() LIMIT 1) as vote_2024,
    ...
    (SELECT json_array_elements_text(demo_options) 
     FROM demo_data WHERE demo = 'age_group' ORDER BY random() LIMIT 1) as age_group,
