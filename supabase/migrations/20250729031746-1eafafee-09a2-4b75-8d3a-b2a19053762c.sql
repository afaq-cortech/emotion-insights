...
-- Using a simpler approach that doesn't require actual auth users
WITH demo_data AS (
...
SELECT 
  -- Use a deterministic UUID based on the email to avoid conflicts
  md5(first_name || '.' || last_name || '@email.com')::uuid as user_id,
  first_name,
  last_name,
